...
def reduce_from_tensor_model_parallel_region(input_, group=None):
    ...
    if group is not None and torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1:
      output = torch.distributed.all_reduce(input_, op=torch.distributed.ReduceOp.SUM, group=group)
      return output
    return input_
...
class ColumnParallelLinear(nn.Linear):
    ...
    def __init__(
        ...
        """
        Initialize a ColumnParallelLinear layer.

        Args:
            in_features (int): Number of input features.
            out_features (int): Number of output features.
            bias (bool, optional): Whether to include bias. Defaults to True.
            gather_output (bool, optional): Whether to gather output across
                model parallel ranks. Defaults to True.
            init_method (Callable, optional): Initialization method for
                the weights. Defaults to None.
            group (torch.distributed.ProcessGroup, optional): The process group
                for distributed communication. Defaults to None.
        """
        self.group = group
        ...
        if torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1:
          ...
          super().__init__(in_features, local_out_features, bias)
        else:
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if self.gather_output:
           output = gather_from_tensor_model_parallel_region(output, self.group)
        return output
...
class RowParallelLinear(nn.Linear):
    ...
    def __init__(
        ...
        """
        Initialize a RowParallelLinear layer.

        Args:
            in_features (int): Number of input features.
            out_features (int): Number of output features.
            bias (bool, optional): Whether to include bias. Defaults to True.
            input_is_parallel (bool, optional): Whether input is already
                split across model parallel ranks. Defaults to True.
            init_method (Callable, optional): Initialization method for the
        ...
        if torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1:
            ...
            super().__init__(local_in_features, out_features, bias)
        else:
            super().__init__(in_features, out_features, bias)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if self.input_is_parallel:
          output = F.linear(input, self.weight, self.bias)
          output = reduce_from_tensor_model_parallel_region(output, self.group)
          return output
...
class VocabParallelEmbedding(nn.Embedding):
    ...
    def __init__(
        ...
        """
        Initialize a VocabParallelEmbedding layer.
        ...
        if torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1:
            world_size = torch.distributed.get_world_size(group=group)
            rank = torch.distributed.get_rank(group=group)
            local_num_embeddings = num_embeddings // world_size
            padding_idx = None if num_embeddings % world_size == 0 else 0
            super().__init__(local_num_embeddings, embedding_dim, padding_idx=padding_idx)
        else:
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1:
            ...
            start_index = self.weight.shape[0] * rank
            end_index = self.weight.shape[0] * (rank+1)
            mask = (input >= start_index) & (input < end_index)
            local_input = torch.where(mask, input - start_index, 0)
            output = F.embedding(local_input, self.weight, self.padding_idx, self.max_norm,
                    self.norm_type, self.scale_grad_by_freq, self.sparse)
            output = torch.where(mask.unsqueeze(-1), output, torch.zeros_like(output))
            return output
        else:
            output = F.embedding(input, self.weight, self.padding_idx, self.max_norm,
                    self.norm_type, self.scale_grad_by_freq, self.sparse)
            return output
...
class ColumnParallelConv2dPatch(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = gather_from_tensor_model_parallel_region(x, self._linear.group)
        return x
...
class ImageFeedForward(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        hidden = reduce_from_tensor_model_parallel_region(hidden, self.group)
...
class ImageAttention(nn.Module):
    def __init__(
        ...
        super().__init__()
        if group is not None and torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1:
            model_parallel_size = torch.distributed.get_world_size(group=group)
            qkvo_replication = 1
    ...
    def forward(
        ...
        out = reduce_from_tensor_model_parallel_region(out, self.group)
...
class VisionEncoder(nn.Module):
    def __init__(
        ...
        self.transformer = ImageTransformer(
            width, layers, heads, mlp_ratio, act_layer=act_layer, group=group
        )
        # pre and post tile position embedding
        self.global_transformer = ImageTransformer(
            width, n_global_layers, heads, mlp_ratio, act_layer=act_layer, gated=True, group=group
        )
        # pre and post tile position embedding
...
class Attention(nn.Module):
    ...
    def __init__(self, args: ModelArgs, group=None):
        """
...
class FeedForward(nn.Module):
    def __init__(
        ...
        self.w1 = ColumnParallelLinear(
            dim, hidden_dim, bias=False, gather_output=False, init_method=lambda x: x, group=group
        )
        self.w2 = RowParallelLinear(
            hidden_dim, dim, bias=False, input_is_parallel=True, init_method=lambda x: x, group=group
        )
...
class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs, group=None):
        """
        ...
        self.dim = args.dim
        self.head_dim = args.dim // args.n_heads
        self.attention = Attention(args, group=group)
...
class CrossAttentionTransformerVision(torch.nn.Module):
    def __init__(self, args: ModelArgs, group=None) -> None:
        super().__init__()
        self.group = group
    ...
    def forward(
        ...
        vision_tokens = gather_from_tensor_model_parallel_region(vision_tokens, self.group)
...
class CrossAttentionTransformerText(torch.nn.Module):
    ...
    def __init__(self, args: ModelArgs, group=None) -> None:
        ...
        self.n_kv_heads = args.n_heads if args.n_kv_heads is None else args.n_kv_heads
        self.n_local_kv_heads = self.n_kv_heads // model_parallel_size
        assert self.vocab_size % model_parallel_size == 0
        self.tok_embeddings = VocabParallelEmbedding(
            args.vocab_size, args.dim, init_method=lambda x: x, group=group
        )
        ...
        self.output = ColumnParallelLinear(
            args.dim, args.vocab_size, bias=False, init_method=lambda x: x, group=group
        )
        ...
        for i in range(args.n_layers):
            layer_id = i
            block = TransformerBlock(args=args, layer_id=layer_id, group=group)
...
class CrossAttentionTransformer(torch.nn.Module):
    def __init__(self, args: ModelArgs, group=None) -> None:
        ...
        self.model_dim = args.dim
        self.vision_model = CrossAttentionTransformerVision(args, group=group)
        self.text_model = CrossAttentionTransformerText(            args, group=group
        )
        self.image_res = args.vision_chunk_size
        self.max_num_chunks = args.vision_max_num_chunks
